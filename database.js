...
module.exports.connector = async () => {
  try {
    if (retry < maxRetry) {
      ...
      console.log("MongoDB max try connect error!");
      return null;
    }
  } catch (error) {
    console.error("MongoDB connection error:", error);
    retry++;
    await new Promise((resolve) => setTimeout(resolve, 1000)); // Yeniden denemeden önce bir süre bekle
    return await this.connector(); // Bağlantıyı yeniden dene
  }
};
